...
export default async function CallsListPage() {
  ...
  const initialCalls: Call[] =
    callsResult.success && callsResult.data ? callsResult.data : [];
  ...
  const initialTotal =
    totalResult.success && totalResult.data
      ? totalResult.data.length
      : initialCalls.length;

  const costResult = hasActivePackage
    ? null
    : await getTotalCostCents(session.user.id);
